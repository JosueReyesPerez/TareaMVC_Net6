...
async function borrarPaso(paso) {
    ...
    tareaEditarViewModel.pasos.remove(function (item) {
...
async function actualizarOrdenPasos() {
    ...
    const arregloOrganizado = tareaEditarViewModel.pasos.sorted((a, b) => {
...
async function enviarIdsPasosAlBackend(ids) {
    ...
    var response = await fetch(`${urlPasos}/ordenar/${tareaEditarViewModel.id}`, {
        method: 'POST',
        body: data,
        headers: {
            'Content-Type': 'application/json'
